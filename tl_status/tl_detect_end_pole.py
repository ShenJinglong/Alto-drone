...
class tl_detect_end_pole_fsm(base_fsm):
    def enter_state(self, flight):
        flight.median_filter.refresh()

    def exec_state(self, flight):
        tl_detect_end_pole(flight)

    def exit_state(self, flight):
        pass

tl_detect_end_pole_counter = 0

def tl_detect_end_pole(flight):
    global tl_detect_end_pole_counter
    ...
    if tl_detect_end_pole_counter == tl_params.TL_DETECT_END_POLE_NUM:
        tl_detect_end_pole_counter = 0