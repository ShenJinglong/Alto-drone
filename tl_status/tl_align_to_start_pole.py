...
class tl_align_to_start_pole_fsm(base_fsm):
    def enter_state(self, flight):
        flight.median_filter.refresh()

    def exec_state(self, flight):
        tl_align_to_start_pole(flight)

    def exit_state(self, flight):
        pass

tl_align_to_start_pole_counter = 0

def tl_align_to_start_pole(flight):
    global tl_align_to_start_pole_counter
    ...
    if tl_align_to_start_pole_counter == tl_params.TL_ALIGN_TO_START_POLE_NUM:
        tl_align_to_start_pole_counter = 0
        flight.next_state = macro.TL_DETECT_BAR_CODE
        print('>>> [!] TL_ALIGN_TO_START_POLE -> TL_DETECT_BAR_CODE')