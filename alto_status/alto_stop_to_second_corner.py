...
class alto_stop_to_second_corner_fsm(base_fsm):
    def enter_state(self, flight):
        flight.median_filter.refresh()

    def exec_state(self, flight):
        alto_stop_to_second_corner(flight)

    def exit_state(self, flight):
        pass

alto_stop_to_second_corner_counter = 0

def alto_stop_to_second_corner(flight):
    global alto_stop_to_second_corner_counter
    ...
    if len(lines_params) > 1:
        ...
        if len(cross_points) == 1:
            ...
            flight.median_filter.add_point_c1([global_params.IMAGE_CENTER_X - 15, global_params.IMAGE_CENTER_Y + 15])
    else:
        flight.median_filter.add_point_c1([global_params.IMAGE_CENTER_X - 15, global_params.IMAGE_CENTER_Y + 15])
    ...
    # alto_stop_to_second_corner_counter += 1
    if alto_stop_to_second_corner_counter == alto_params.ALTO_STOP_TO_SECOND_CORNER_NUM:
        alto_stop_to_second_corner_counter = 0
        flight.next_state = macro.ALTO_BACK
        print('>>> [!] ALTO_STOP_TO_SECOND_CORNER -> ALTO_BACK')
